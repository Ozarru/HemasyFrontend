...
function ConsultationsGenres({ cons_genres }) {
  ...
  const submitForm = async (event) => {
    ...
    let decision = confirm(
      "Voulez vous ajouter ce type de consultation a la base de données?"
    );
    if (decision) {
      ...
      <div className="flex p-4 justify-between">
        ...
        <div className="w-4/6 px-4">
          ...
          <div className="p-8 bg-gradient-to-r from-sky-500 rounded-3xl grid grid-cols-4 gap-4">
            {cons_genres.map((cons_genre) => {
              return (
                <>
                  <Link href={`cons_genre/${cons_genre.id}`} passHref>
                    <div className="p-4 bg-gray-100 rounded-xl text-black">
                      <h2 className="text-md font-bold">{cons_genre.name}</h2>
                      <h2 className="p-1 text-sm text-green-500 font-semibold bg-white rounded-lg">
                        {cons_genre.price} CFA
                      </h2>
                      <h2 className="text-sm font-bold">Parametres requis:</h2>
                      <p className="text-sm text-gray-500">
                        {cons_genre.parameters}
                      </p>
                    </div>
                  </Link>
                </>
          ...
          <h1 className="text-left py-4 max-w-lg text-lg font-bold">
            Liste des types de Consultations
          </h1>
          <div className="overflow-auto rounded-xl bg-white">
            {cons_genres.length === 0 ? (
                    ...
                    return (
                      <>
                        <tr className="text-gray-500 hover:text-black cursor-pointer transition-all duration-50 text-sm">
                          <td className="bg-ligthest p-3">{cons_genre.name}</td>
        ...
        <div className="text-left px-4 w-2/6">
          <h1 className="pb-4 text-lg font-bold">
            Formulaire des Types de Consultation
          </h1>
          <form
            action="#"
            method="POST"
          ...
          >
            ...
            <div>
              ...
              <textarea
                ...
                type="text"
                name="parameters"
                placeholder="paramètres de santé"
              ></textarea>
            </div>
...
export default ConsultationsGenres;
...
export async function getServerSideProps() {
  ...
  return { props: { cons_genres: consultations } };
}
