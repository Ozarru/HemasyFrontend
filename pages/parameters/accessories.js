...
function Accessories({ accessories, lots }) {
  ...
  const submitForm = async (event) => {
    ...
    let decision = confirm(
      "Voulez vous enregistrer ce type d'Accessoires dans la base de données?"
    );
    if (decision) {
    ...
    <>
      <div className="flex p-4 justify-between">
          ...
          <div className="p-4 bg-gradient-to-l from-sky-500 rounded-3xl">
            {Array.from(accessories).length === 0 ? (
              ...
              <div className=" grid grid-cols-4 gap-2">
                {Array.from(accessories).map((accessory) => {
                  return (
                    <>
                      <Link href={`accessory/${accessory.id}`} passHref>
              ...
              <table className="w-full text-left db-table">
                <thead className="bg-blue-900 text-white">
                  <tr>
                    ...
                    <th className="p-4 text-sm font-se tracking-wide">Usage</th>
                  </tr>
                </thead>
                <tbody>
                  {Array.from(accessories).map((accessory) => {
                    return (
                      <>
                        <tr className="text-gray-500 hover:text-black cursor-pointer transition-all duration-50 text-sm">
                          <td className="bg-ligthest p-3">{accessory.name}</td>
                          ...
                          <td className="bg-ligthest p-3">{accessory.usage}</td>
                        </tr>
                      </>
                    );
                  })}
        ...
        <div className="text-left px-4 w-2/6">
          <h1 className="pb-4 text-lg font-bold">Formulaire des Accessoires</h1>
          <form
            action="#"
            method="POST"
            onSubmit={submitForm}
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                type="text"
                name="name"
                placeholder="nom de la Accessoires"
                required
              ></input>
            </div>
            <div>
...
export async function getServerSideProps() {
  ...
  return { props: { accessories: data, lots: lot_data } };
}
